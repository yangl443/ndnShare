...
LinkService::sendInterest(const Interest& interest)
{
  ...
  NFD_LOG_FACE_TRACE("yangl433-sendInterest-Name:" << interest.getName());
  ++this->nOutInterests;
...
LinkService::sendData(const Data& data)
{
  BOOST_ASSERT(m_transport != nullptr);
  NFD_LOG_FACE_TRACE(__func__);
  NFD_LOG_FACE_TRACE("yangl433-senddata-Name:" << data.getName());
  ++this->nOutData;
...
{
  NFD_LOG_FACE_TRACE(__func__);
    NFD_LOG_FACE_TRACE("yangl433-receive-data-Name:" << data.getName());
  ++this->nInData;
